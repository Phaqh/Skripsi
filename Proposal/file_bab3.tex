...
\begin{figure}[!htbp]
    ...
    \caption{Finite Automata untuk strategi IPD.}\label{fig:dfa}
\end{figure}
...
Pada formulasi di atas, $t$ menyatakan waktu keputusan saat ini, sedangkan $H \in \mathbb{N}$ adalah horizon prediksi tetap yang menentukan panjang lintasan masa depan yang dipertimbangkan. Indeks $k$ merepresentasikan langkah prediksi ke-$k$ relatif terhadap waktu $t$. Simbol $\hat{a}_{t+k}^{-i}$ menyatakan aksi lawan yang diprediksi pada waktu $t+k$, dengan superskrip $-i$ menunjukkan pemain lawan terhadap agen $i$. Distribusi prediktif $p_\theta(\cdot \mid \hat{h}_{t+k-1})$ merupakan model parametrik dengan parameter $\theta$ yang mengaproksimasi probabilitas aksi lawan bersyarat pada riwayat yang tersedia. Notasi $\hat{h}_{t+k-1}$ menyatakan riwayat interaksi yang telah diperluas secara rekursif hingga waktu $t+k-1$, termasuk aksi-aksi yang sebelumnya diprediksi atau direalisasikan selama proses rollout.
