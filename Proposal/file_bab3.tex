...
Dalam konteks iterated game dua pemain, history hingga waktu $t$ tidak hanya terdiri dari aksi satu pemain, tetapi pasangan aksi kedua pemain pada setiap putaran. 
...
Dalam pengaturan iteratif, history tidak tersedia secara sekaligus, melainkan terbentuk secara \textit{online}. Pada setiap putaran $t$, history diperbarui secara inkremental sebagai:
...
Oleh karena itu, permasalahan strategis dalam IPD dengan agen adaptif berada dalam kerangka permainan dengan informasi tidak lengkap, di mana agen harus melakukan inferensi terhadap strategi lawan secara bertahap seiring pertambahan history (\cite{bonanno_game_2024}).
...
Arsitektur berbasis jaringan saraf dapat menangkap dependensi temporal dan nonlinier untuk memodelkan dinamika yang lebih kompleks (\cite{hochreiter_long_1997}).
...
\subsection{Forecasting Rekursif dan Monte Carlo Rollout (Fixed Horizon)}
...
dengan $\hat{h}_{t+k-1}$ merupakan history yang diperluas secara rekursif 
...
di mana $Q_H(a_i \mid h_t)$
merupakan estimasi nilai aksi
berdasarkan rollout Monte Carlo
sepanjang horizon tetap $H$.

Dengan formulasi ini,
dynamic regret mengukur selisih performa agen
terhadap keputusan optimal
yang dihitung ulang pada setiap waktu,
tanpa mengasumsikan strategi pembanding yang statis.

Metrik ini lebih sesuai
untuk mengevaluasi kinerja dalam lingkungan non-stasioner,
karena comparator diperbolehkan berubah secara temporal.
...
$p(a_t^{-i} \mid h_{0:t-1})$
berdasarkan riwayat interaksi $h_{0:t-1}$,
...
Pendekatan multi-step forecasting dilakukan secara rekursif,
di mana prediksi pada waktu $t+1$ digunakan sebagai input
untuk memprediksi waktu $t+2$, dan seterusnya
hingga horizon tetap $H$.
...
dan memperbesar deviasi distribusi pada langkah-langkah berikutnya
(\cite{bengio_scheduled_2015}).

Secara formal, jika model menghasilkan distribusi prediktif

\begin{equation}
\hat{P}(a_{-i}^{t+1} \mid h_t),
\end{equation}

maka distribusi pada langkah ke-$k$ diberikan oleh

\begin{equation}
\hat{P}(a_{-i}^{t+k} \mid \hat{h}_{t+k-1}),
\end{equation}

dengan $\hat{h}_{t+k-1}$ merupakan history yang diperluas
menggunakan prediksi sebelumnya.
Karena $\hat{h}_{t+k-1}$ sendiri merupakan hasil prediksi model,
maka terjadi \textit{distribution shift}
antara distribusi pelatihan dan distribusi saat inferensi rekursif.

Dalam kerangka horizon tetap $H$,
nilai suatu aksi $a_t$ didefinisikan sebagai ekspektasi
akumulasi payoff sepanjang $H$ langkah ke depan:
...
Formulasi ini menunjukkan bahwa evaluasi keputusan pada waktu $t$
tidak hanya bergantung pada prediksi satu langkah,
melainkan pada kualitas estimasi distribusi lintasan
sepanjang horizon terbatas $H$.
Akumulasi kesalahan prediksi dalam forecasting rekursif
dapat menyebabkan bias pada estimasi $Q_H$,
sehingga stabilitas pelatihan menjadi aspek krusial
dalam pendekatan multi-langkah.
...
dengan $h_t$ hidden state,
$y_{t-1}$ input autoregresif,
$\hat{y}_t$ prediksi model,
dan $\theta$ parameter yang dipelajari.

Dalam skema \textit{teacher forcing},
input autoregresif selama pelatihan digantikan
oleh ground-truth $y_{t-1}^{*}$:
...
identik dengan distribusi data sebenarnya.
...
Meskipun \textit{teacher forcing} mempercepat konvergensi pelatihan, 
pendekatan tersebut menimbulkan perbedaan distribusi antara fase pelatihan dan inferensi 
yang dikenal sebagai \textit{exposure bias}. 
Pada saat inferensi, model tidak lagi menerima ground-truth $y_{t-1}^{*}$, 
melainkan prediksi sebelumnya $\hat{y}_{t-1}$, sehingga kesalahan dapat terakumulasi secara rekursif.

Untuk menjembatani kesenjangan distribusi tersebut, 
\textit{scheduled sampling} memperkenalkan mekanisme pencampuran 
antara ground-truth dan prediksi model selama pelatihan.
...
\tilde{y}_{t-1} =
\begin{cases}
y_{t-1}^{*}, & \text{dengan probabilitas } \epsilon_k \\
\hat{y}_{t-1}, & \text{dengan probabilitas } 1 - \epsilon_k
\end{cases}
\end{equation}

dengan $\epsilon_k \in [0,1]$ merupakan probabilitas penggunaan teacher forcing 
pada iterasi pelatihan ke-$k$.

Dinamika hidden state menjadi:

\begin{equation}
h_t = f(h_{t-1}, \tilde{y}_{t-1}; \theta)
\end{equation}
...
Nilai $\epsilon_k$ dijadwalkan menurun secara bertahap agar model 
secara progresif bertransisi dari rezim pelatihan berbasis ground-truth 
menuju rezim inferensi berbasis prediksi sendiri.
...
\begin{equation}
\epsilon_k = \epsilon_0 \gamma^k
\end{equation}

\textbf{Inverse Sigmoid Decay}
\begin{equation}
\epsilon_k =
\frac{\kappa}{\kappa + \exp\left(\frac{k}{\kappa}\right)}
\end{equation}

dengan $\epsilon_0$ probabilitas awal teacher forcing, 
$\alpha$ laju penurunan linear, 
$\gamma \in (0,1)$ faktor peluruhan eksponensial, 
dan $\kappa$ parameter bentuk kurva.

Dengan mekanisme ini, distribusi input selama pelatihan secara bertahap 
mendekati distribusi saat inferensi, 
sehingga model menjadi lebih stabil terhadap propagasi kesalahan 
pada prediksi multi-langkah.
...
\section{Optimisasi}
\subsection{Pelatihan LSTM dengan Backpropagation Through Time}

Parameter LSTM $\theta = \{W, U, b\}$ dipelajari melalui minimisasi fungsi kerugian berbasis likelihood. 
Dalam konteks pemodelan lawan, model menghasilkan distribusi probabilitas atas aksi lawan pada setiap waktu $t$:

\begin{equation}
p_\theta(a_{-i}^{t} \mid h_{t-1})
\end{equation}
...
\begin{equation}
\mathcal{L}(\theta) 
= - \sum_{t=1}^{T} \log p_\theta(a_{-i}^{t} \mid h_{t-1})
\end{equation}

Untuk memperbarui parameter $\theta$, gradien $\nabla_\theta \mathcal{L}$ dihitung menggunakan algoritma \textit{Backpropagation Through Time} (BPTT). 
...
memungkinkan aliran gradien yang lebih stabil dibandingkan RNN konvensional, karena turunan terhadap $c_{t-1}$ mengandung komponen linear melalui faktor $f_t$. Struktur ini secara signifikan mengurangi permasalahan \textit{vanishing gradient} dalam pembelajaran dependensi jangka panjang.

Dalam praktiknya, untuk efisiensi komputasi dan stabilitas numerik, BPTT sering diterapkan dalam bentuk \textit{truncated BPTT}, di mana propagasi gradien dibatasi pada jendela waktu sepanjang $K < T$. Parameter kemudian diperbarui menggunakan algoritma optimisasi berbasis gradien seperti Stochastic Gradient Descent (SGD) atau Adam.